...
int step;
int maxGrade, minOpponent, maxPreGrade;
...
int Evaluate() {
    ...
    for(int r=0; r<SIZE; r++) {
        for(int c=0; c<SIZE; c++) {
            ...
            //relatively conquer
            if(stepB - stepW > 0) GradeW+= 20;
            if(stepW - stepB > 0) GradeB+= 20;

            //partly conquer
            if(stepB - stepW > 1) GradeW+= 20;
            if(stepW - stepB > 1) GradeB+= 20;

            //mostly conquer
            if(stepB - stepW > 2) GradeW+= 20;
            if(stepW - stepB > 2) GradeB+= 20;
...
void searchArrow(Position from, Position to) {
    ...
    for(int r=0; r<SIZE; r++)   {
        for(int c=0; c<SIZE; c++) {
            ...
            if(validArrow(to, arrow)) {
                //if(Search(arrow, Opn) > 1) continue;
